...
export function FAQ({ onJoin }: { onJoin: () => void }) {
  ...
  return (
    <section
      id="faq"
      className="relative overflow-hidden bg-slate-50 px-6 pb-24 pt-14 sm:pb-32 sm:pt-20 lg:px-12"
    >
        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-[1fr_1.5fr] lg:gap-24">
          ...
          <div className="flex flex-col items-start lg:sticky lg:top-32 lg:h-max">
            <div className="inline-flex items-center rounded-full border border-teal-600/10 bg-teal-50 px-3 py-1 text-sm font-semibold text-teal-700">
              <span className="flex h-1.5 w-1.5 rounded-full bg-teal-600 mr-2" />
            ...
            <button
              onClick={onJoin}
              className="group mt-10 relative inline-flex h-12 items-center justify-center overflow-hidden rounded-full bg-teal-600 px-8 font-semibold text-white transition-all duration-300 hover:bg-teal-700 hover:shadow-[0_0_40px_-10px_rgba(13,148,136,0.6)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-600 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-50"
            >
              ...
              return (
                <div
                  ...
                  className={classNames(
                    ...
                    isOpen
                      ? "border-teal-200 bg-white shadow-lg shadow-teal-900/5 ring-1 ring-teal-900/5"
                      : "border-slate-200 bg-white/60 hover:bg-white hover:shadow-md"
                  )}
                >
                  <button
                    onClick={() => setOpenIdx((prev) => (prev === idx ? -1 : idx))}
                    className="flex w-full items-center justify-between gap-6 px-6 py-5 text-left focus-visible:outline-none rounded-2xl focus-visible:ring-2 focus-visible:ring-teal-600 focus-visible:ring-inset"
